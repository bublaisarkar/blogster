...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      // ✅ Show success toast
      toast.success('Registration successful! Welcome to Blogster!');
      navigate('/dashboard');
    } else {
      // ✅ Show error toast
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-[#edebf5] p-6 sm:p-8 md:p-10">
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-indigo-100 rounded-2xl mb-4">
                <i className="fas fa-user-plus text-indigo-600 text-3xl"></i>
              </div>
              <h1 className="text-2xl sm:text-3xl font-bold text-[#14141f]">Create an account</h1>
              <p className="text-[#6b6b84] mt-2 text-sm">Join the Blogster community today</p>
            </div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                <label htmlFor="name" className="block text-sm font-medium text-[#2d2d3f] mb-1.5">
                  <i className="fas fa-user mr-2 text-indigo-500"></i> Full Name
                </label>
                <input 
                  type="text" 
                  id="name" 
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={handleChange}
                  placeholder="••••••••" 
                  className={`w-full px-4 py-3 rounded-xl border ${passwordError && formData.confirmPassword ? 'border-red-500' : 'border-[#e6e6ed]'} bg-[#faf9f6] focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition text-sm`}
                  required
                  disabled={loading}
                />
                <p className="text-xs text-[#6b6b84] mt-1.5">Must be at least 8 characters</p>
              </div>

              {/* Confirm Password */}
              <div>
                <label htmlFor="confirmPassword" className="block text-sm font-medium text-[#2d2d3f] mb-1.5">
                  <i className="fas fa-check-circle mr-2 text-indigo-500"></i> Confirm Password
                </label>
                <input 
                  type="password" 
                  id="confirmPassword" 
                  ...
                  onChange={handleChange}
                  placeholder="••••••••" 
                  className={`w-full px-4 py-3 rounded-xl border ${passwordError ? 'border-red-500' : 'border-[#e6e6ed]'} bg-[#faf9f6] focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition text-sm`}
                  required
                  disabled={loading}
                />
              </div>
              ...
              <div className="flex items-start gap-2 pt-1">
                <input 
                  ...
                  onChange={handleChange}
                  className="w-4 h-4 mt-0.5 rounded border-[#e6e6ed] text-indigo-600 focus:ring-indigo-500 focus:ring-offset-0 transition flex-shrink-0"
                  required
                  disabled={loading}
                />
                <label htmlFor="terms" className="text-sm text-[#2d2d3f]">
                  I agree to the 
                  <Link to="/terms" className="text-indigo-600 hover:text-indigo-800 transition font-medium ml-1">Terms of Service</Link> 
                  and 
                  <Link to="/privacy" className="text-indigo-600 hover:text-indigo-800 transition font-medium ml-1">Privacy Policy</Link>
                </label>
              </div>
              ...
              <p className="text-center text-sm text-[#6b6b84] mt-4">
                ...
                <Link to="/login" className="text-indigo-600 hover:text-indigo-800 font-medium transition ml-1">
                  Sign in
                </Link>
              </p>
            </form>
